# Requires https://github.com/murraylab/PsychRNN
# pip install psychrnn
# THis runs, but it does not apparently use the GPU. I do not know why. Might be the environment. It makes slight use of the GPU in the tf-GPU environment.
# not that much faster thougth.
#
# UPSHOT: I get all of this to run just fine. 
# TODO: need to adjust to model OUR data
# TODO: Need to get to allow to define the proportion of negative and positive weights.
...
train_params['training_iters'] = 50000 # number of iterations to train for Default: 50000, use more if not optimal
...
plt.figure()
#plt.imshow(weights['W_rec'], extent=[0, 1, 0, 1])
plt.imshow(weights['W_rec'])
plt.colorbar()
plt.title("W_rec")
plt.set_cmap('bwr') # see https://matplotlib.org/stable/tutorials/colors/colormaps.html

plt.figure()
plt.imshow(weights['W_in'])
plt.colorbar()
plt.title("W_in")
plt.set_cmap('bwr') # see https://matplotlib.org/stable/tutorials/colors/colormaps.html

plt.figure()
plt.imshow(weights['W_out'])
plt.colorbar()
plt.title("W_out")
plt.set_cmap('bwr') # see https://matplotlib.org/stable/tutorials/colors/colormaps.html