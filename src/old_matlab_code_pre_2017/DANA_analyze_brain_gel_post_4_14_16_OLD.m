...
for iD = 1:length(don)
    ...
    if PLOT_IT
        ...
        xlabel('sec')
        saveas(gcf,sprintf('SinglesubplotExample%s.eps',tit))
    ...
    [PW_FSCVOFF] = pwelch(LFP_off,[],[],welch_fq,sFreq); % let welch decide.
    PW_before = PW_FSCVOFF; % remember that this is already log scaled.
    
    [PW_snr_before] = pwelch(LFP_off,[],[],SnR_FQ,sFreq); % let welch decide.
    SnR_before_welch = PW_snr_before(2)/mean(PW_snr_before([1 3]));

    [PW_FSCVON] = pwelch(LFP_on,[],[],welch_fq,sFreq); % let welch decide.
    PW_after = PW_FSCVON; % remember that this is already log scaled.
    [SnR_PWR_after_welch_all] = pwelch(LFP_on,[],[],SnR_FQ,sFreq); % let welch decide.
    ...
    if PLOT_IT
        ...
        legend('PW FSCVOFF SIGOFF','PW FSCVON SIGOFF')
        xlabel('Hz')
        ylabel('Power dB/Hz (10log10)','Interpreter','Latex')
        %         pubify_figure_axis
        saveas(gcf,sprintf('Example%s.eps',tit))

    end
    
    S_fq = Frequency_range_to_examine_BLK(2): Frequency_range_to_examine_BLK(3);
    [~,fq,~,P] = spectrogram(LFP_off,window_samples_spectrogram,round(window_samples_spectrogram/2),S_fq,sFreq);
    ...
    samples_after = LFP_sFreq*.180;skip=1;
    % For slower frequencies.
    samples_after = LFP_sFreq*1.90;skip = 10;
    ...
    for iScan = 1:Cols(L)
        ...
        [PW] = pwelch( v, sum(welch_IX), [], welch_fq, LFP_sFreq); % plot(welch_fq,PW)
        PWn = PW - PW_before;
        PWlog = 10*log10(PW);
