function Q3_does_stim_affect_ensemble_act_Ana(ana_dir)
%%
close all
clrs = lines(200);
if isempty(ana_dir)
...
for iN = 1:length(good_cluster_IDs)
    ...
    for ii = 1:length(uHz)
        ...
        M = M - M(:,1);
        subplot_ij(length(good_cluster_IDs),length(uHz),iN,ii)
...
for iN = 1:length(good_cluster_IDs)
    for ii = 1:length(uLVs)
        ...
        subplot_ij(length(good_cluster_IDs),length(uLVs),iN,ii)
        boxplot(M,'notch','on')
        ylabel('LV')
